...
/**
 * Main crawling function - crawls all sites and generates docs index
 * @returns {Promise<Object>} Documentation index with all sites
 */
export async function crawlAllSites() {
  console.log('Starting documentation crawling...');
 ...
 * @param {string} outputPath - Path to save the index file
 */
export async function generateDocsIndex(outputPath = './src/data/docs-index.json') {
  try {
    const index = await crawlAllSites();
    ...
    console.log(`Documentation index saved to: ${outputPath}`);
    
    // Generate summary
    let totalPages = 0;
    for (const site of Object.values(index)) {
      totalPages += site.pages.length;
    }
    
    console.log(`Summary: ${Object.keys(index).length} sites, ${totalPages} total pages`);
